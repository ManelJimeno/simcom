...
{
    unsigned short port = static_cast<unsigned short>(config().getUInt("server.port"));
    Address address(config().getString("server.host"), port);
    auto connections = config().getInt("server.connections");
    auto delay = config().getInt("server.delay");
    auto parallel = config().getInt("server.parallel");
    auto counter = 0;
    auto batch = 1;
